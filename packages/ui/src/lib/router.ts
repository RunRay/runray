...
export function parseHash(hash: string): Route {
  ...
  if (segments[0] === 'run' && segments[1] !== undefined) {
    ...
    if (segments[2] === 'waste') return { view: 'waste', runId };
...
export function toHash(route: Route): string {
  switch (route.view) {
    ...
    case 'waste':
      return `#/run/${encodeURIComponent(route.runId)}/waste`;
    case 'cost':
