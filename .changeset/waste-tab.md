...
- `@runray/core/waste` (browser-safe): a run's findings grouped by rule
  and split by class — burned (the engine's capped waste-class total) and
  opportunities (upper bounds, never added to it) — each group graded on
  its sum by the engine's own severity tiers, groups whose findings are
  all under $0.05 folded, a cache break's shape and tokens read from its
  two evidence calls, and every burned finding placed on the session's
  clock next to the context size of each model call. `gradeSeverity` and
  the break shape move to pure modules shared with the rule engine.
