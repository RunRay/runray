...
- **Timeline → findings strip.** The five largest findings, ranked by amount, tinted by severity, then *+N more in Waste*. Click a pill to highlight its evidence rows and open it in the Inspector.
...
- **Waste tab.** A session's findings grouped by rule, in two figures kept apart: *burned* (the engine's waste-class total, capped at the session's cost, with its share of the session) and *opportunities* (upper bounds that do not add up). One sentence names the largest burn and the lever you have for it in your tool; the leak rail places every burned finding on the session's clock over the context each model call carried, with idle gaps shaded and compactions marked; then one row per rule, graded on the group's sum — eighteen small cache breaks read as the 12% they add up to — opening to its occurrences (each a jump to the finding on the timeline), a cache break's split by shape (history re-written · front changed · compaction), and the rule's playbook for your tool. Findings under $0.05 each fold into one line. The *Growing context* row opens to how its estimate is counted — the baseline (median context of the first three calls), the calls counted, the excess tokens and the rate — as an upper bound that assumes nothing about compacting, and to what keeping the context under 100k, 200k or 400k tokens would have saved by the same formula. The what-if repricing closes the tab.
