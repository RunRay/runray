...
Findingi jednej sesji są listą płaską — na realnych sesjach to osiemnaście prawie identycznych przerwań cache i trzynaście re-readów za grosze obok jednej górnej granicy „growing context". `wasteRun(run)` nie wykrywa nic nowego: układa findingi silnika po regule, dzieli klasą z rejestru `RULE_META` na **burned** (waste, nagłówek = `totals.costUSD.wastedEstimate`, z capem) i **opportunities** (Σ estymat, górne granice, nigdy dodawane do burned), ocenia każdą grupę **od sumy** tą samą funkcją `gradeSeverity` i tymi samymi progami, którymi silnik ocenia pojedynczy finding (`insights/severity.ts`, moduł czysty), zwija grupę, której wszystkie findingi są poniżej `warningFloorUSD`, a kształt przerwania cache (`compaction` · `front` · `history`) czyta z dwóch spanów dowodu wspólną funkcją `breakShape` (`insights/cache-shape.ts`), tą samą, którą reguła słowuje finding. Dla klasy waste emituje zdarzenia na osi sesji (`burn` w chwili przerwania lub pierwszego dowodu, `compaction` osobno, `idle` jako pasmo od końca wywołania przed przerwą do wznowienia) i listę kontekstu każdego wywołania modelu (input + cache read + cache write), z których UI rysuje oś wycieków. Czysta funkcja runu, deterministyczna w każdym porządku; WeakMap-cache po stronie UI (`lib/waste.ts`). Analiza na 72 realnych sesjach użytkownika (2026-09-04): 20 sesji z ≥10 findingami, w 9 suma estymat przekracza koszt sesji.
