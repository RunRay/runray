...
### 2.6 Waste grouping (`waste/`, subpath `@runray/core/waste`)

Findingi jednej sesji są listą płaską — na realnych sesjach to osiemnaście prawie identycznych przerwań cache i trzynaście re-readów za grosze obok jednej górnej granicy „growing context". `wasteRun(run)` nie wykrywa nic nowego: układa findingi silnika po regule, dzieli klasą z rejestru `RULE_META` na **burned** (waste, nagłówek = `totals.costUSD.wastedEstimate`, z capem) i **opportunities** (Σ estymat, górne granice, nigdy dodawane do burned), ocenia każdą grupę **od sumy** tą samą funkcją `gradeSeverity` i tymi samymi progami, którymi silnik ocenia pojedynczy finding (`insights/severity.ts`, moduł czysty), zwija grupę, której wszystkie findingi są poniżej `warningFloorUSD`, a kształt przerwania cache (`compaction` · `front` · `history`) czyta z dwóch spanów dowodu wspólną funkcją `breakShape` (`insights/cache-shape.ts`), tą samą, którą reguła słowuje finding. Dla klasy waste emituje zdarzenia na osi sesji (`burn` w chwili przerwania lub pierwszego dowodu, `compaction` osobno, `idle` jako pasmo od końca wywołania przed przerwą do wznowienia) i listę kontekstu każdego wywołania modelu (input + cache read + cache write), z których UI rysuje oś wycieków. Czysta funkcja runu, deterministyczna w każdym porządku; WeakMap-cache po stronie UI (`lib/waste.ts`). Formuła nadwyżki kontekstu reguły `context-bloat` (od czwartego wywołania głównego zakresu, tokeny ponad próg po stawce, jaką to wywołanie faktycznie zapłaciło za token wejściowy) mieszka w `insights/context-excess.ts`: reguła liczy nią estymatę względem baseline (mediana kontekstu trzech pierwszych wywołań), a `contextCapEstimates(run, pricing?)` powtarza ją z pułapem 100k / 200k / 400k zamiast baseline, żeby zakładka Waste powiedziała wprost, ile dałoby utrzymanie kontekstu poniżej pułapu; bez tabeli cen tokeny zostają, kwoty są `undefined`. Analiza na 72 realnych sesjach użytkownika (2026-09-04): 20 sesji z ≥10 findingami, w 9 suma estymat przekracza koszt sesji.
...
- Ładowanie danych: `window.__RUNRAY_DATA__`/`__RUNRAY_PRICING__`/`__RUNRAY_VIEW_CONFIG__` (export) → fallback `fetch('/api/...')` (serve); brak payloadu cennika ⇒ powierzchnie repricingu pokazują notę, nigdy nie fabrykują stawek. Jeden store zustand (+ slice'y `pricing`, `limit`); UI importuje runtime WYŁĄCZNIE z browser-safe subpathów `@runray/core/pricing`, `.../insights-meta`, `.../diff`, `.../triage` i `.../waste` (X1). Zakładka **Errors** (`#/run/:id/errors`, `ErrorsView`) renderuje `triageRun` z core: podsumowanie (ile wymaga użytkownika, ile wróciło do działania, koszt reakcji), pasek właścicieli, oś czasu porażek, klastry per właściciel z tekstem błędu, wystąpieniami (skok do Timeline Explorer przez `selectSpan` + `navigateTo`), findingami (`showInsight`) i playbookiem klasy dla źródła sesji; grupa „Expected feedback" zwinięta i przygaszona. Pill z liczbą błędów (tabela sesji, lista Explorera, zakładka) bierze ton z `triageTone` (czerwony tylko gdy `needsAttention`); dopisek „· N yours to fix" z `byOwner.you` pokazuje tylko szeroka tabela sesji, wąska lista Explorera i zakładka niosą samą liczbę, a rozbicie właścicieli siedzi w tooltipie — `lib/triage.ts` cache'uje triage per obiekt runu (WeakMap). Zakładka **Waste** (`#/run/:id/waste`, `WasteView`) renderuje `wasteRun` z core: dwie liczby osobno (burned z udziałem w sesji, opportunities jako „up to"), zdanie wiodące o największym wypaleniu i dźwigni dla źródła sesji (`lib/waste.ts`), oś wycieków `LeakRail` (krzywa kontekstu per wywołanie z dziurą na długiej przerwie, słupek na każde spalone zdarzenie o wysokości proporcjonalnej do kwoty, pasma bezczynności, znaczniki kompakcji, linia ~200k; słupek otwiera finding przez `showInsight` + `navigateTo`), grupy burned/opportunities z oceną grupy w `SeverityPill`, wystąpieniami (tokeny dla przerwań cache), podziałem kształtów i playbookiem reguły; grupy groszowe jedną linią; na końcu panel What-if, przeniesiony z Overview razem z tabelą findingów. Pasek findingów pokazuje pięć największych i „+N more in Waste"; badge zakładki niesie kwotę burned w tonie silnika (≥ `warningShare`).
- Drzewo komponentów: `App → TopBar(FilterBar, LimitModeToggle, freshness "as of") / SessionsPane / Overview(SavingsPanel[EvidenceInline], KPI, SpendSection, RankCards, ToolRankCard, WasteBoard) / RunView(InsightsStrip, Waterfall[badge Σ poddrzewa], CostView(NestedTreemap, StackedBar), TimeView, ErrorsView, WasteView(LeakRail, WhatIfPanel)) / Inspector(TranscriptPane)`.
