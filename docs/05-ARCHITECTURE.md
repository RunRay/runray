...
| `context-bloat` | opportunity | mediana `input` ostatnich 3 > `multiplier: 2`× mediany pierwszych 3 ∧ > `minMedianInputTokens: 50k` | **kumulatywna** nadwyżka `Σ max(0, input_i − baza)` po stawce input (stara aproksymacja `(last−first)×3` zaniżała ~`llmCalls/3`×) |
...
| `dead-end-run` | waste | run kończy się błędem (span sesji lub ostatni span) | **ogon po awarii**: llm-calle po ostatnim udanym toolu ze zmianą kodu, minus roszczenia klastrów retry; 100% TYLKO gdy nic nie powstało (`criticalCostUSD: 1`) |
